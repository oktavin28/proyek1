...
- Membaca Dataset
  Langkah pertama dalam data preparation adalah memastikan memiliki data yang bisa diolah. Di sini, dataset dibaca menggunakan 'pd.read_csv()', kemudian 5 baris awalnya ditampilkan dengan df.head(). 

- Memeriksa Informasi Dataset
   Mengecek tipe data, jumlah baris, kolom, dan jumlah nilai non-null untuk setiap kolom. Hal ini membantu mengidentifikasi apakah ada missing values atau tipe data yang tidak sesuai. dengan menggunakan fungsi fungsi `df.info()`

- Memeriksa Duplikasi Data
  Mengidentifikasi jumlah baris duplikat dalam dataset, menggunakan fungsi `df.duplicated().sum()`. Pada data ini menghasilkan 715 baris duplikat.

- Menghapus Data Duplikat
  Menghapus 715 baris duplikat untuk memastikan dataset bersih dari data redundan, menggunakan fungsi `df.drop_duplicates()`

- Identifikasi Missing Values
  Menghitung jumlah Missing Values di setiap kolom, 

- Menghapus Kolom yang tidak Relevan
  Menghapus kolom dianggap yang tidak relevan atau tidak memiliki pengaruh sifnifikan terhadap target prediksi (harga/MSRP). Dalam proyek ini, kolom seperti "Model", "Market Category", dan "Number of Doors" dihapus karena dianggap tidak relevan untuk target  (harga/MSRP).
...
- Bandingkan hasil dari ketiga model untuk memilih model terbaik.
